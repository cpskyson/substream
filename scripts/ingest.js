...
const upsertPoolSql = `
  ...
  on conflict (pool_id) do update set
    ...
    token_a_decimals = case when excluded.token_a_decimals > 0 then excluded.token_a_decimals else pools.token_a_decimals end,
    token_b_decimals = case when excluded.token_b_decimals > 0 then excluded.token_b_decimals else pools.token_b_decimals end,
    fee_tier_bps = case when excluded.fee_tier_bps > 0 then excluded.fee_tier_bps else pools.fee_tier_bps end,
    token_a_balance = case when excluded.token_a_balance > 0 then excluded.token_a_balance else pools.token_a_balance end,
    token_b_balance = case when excluded.token_b_balance > 0 then excluded.token_b_balance else pools.token_b_balance end,
    tvl_estimate = case when excluded.tvl_estimate > 0 then excluded.tvl_estimate else pools.tvl_estimate end,
    tvl_usd = case when excluded.tvl_usd > 0 then excluded.tvl_usd else pools.tvl_usd end,
    token_a_price_usd = case when excluded.token_a_price_usd > 0 then excluded.token_a_price_usd else pools.token_a_price_usd end,
    token_b_price_usd = case when excluded.token_b_price_usd > 0 then excluded.token_b_price_usd else pools.token_b_price_usd end,
...
const upsertPositionSql = `
  ...
  on conflict (position_id) do update set
    ...
    token_a_decimals = case when excluded.token_a_decimals > 0 then excluded.token_a_decimals else positions.token_a_decimals end,
    token_b_decimals = case when excluded.token_b_decimals > 0 then excluded.token_b_decimals else positions.token_b_decimals end,
    fee_tier_bps = case when excluded.fee_tier_bps > 0 then excluded.fee_tier_bps else positions.fee_tier_bps end,
    token_a_balance = case when excluded.token_a_balance > 0 then excluded.token_a_balance else positions.token_a_balance end,
    token_b_balance = case when excluded.token_b_balance > 0 then excluded.token_b_balance else positions.token_b_balance end,
    pool_value = case when excluded.pool_value > 0 then excluded.pool_value else positions.pool_value end,
